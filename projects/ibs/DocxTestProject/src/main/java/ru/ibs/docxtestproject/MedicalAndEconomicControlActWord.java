...
public class MedicalAndEconomicControlActWord {
	...
	public void create(Long lpuId, Date period, Long parcelId) throws IOException {
		...
		if (lpuId.equals(4708L)) {
			headList = Db.select(sessionFactory, session -> session.createSQLQuery("select * from table (pmp_smo1.pmp_akt_mek_V1.INIT_GET_report(:periodStr, :lpuId, :parcelId, null, null))").setParameter("lpuId", lpuId).setParameter("period", period).setParameter("parcelId", parcelId).list());
		} else {
			headList = Db.select(sessionFactory, session -> session.createSQLQuery("select * from table (pmp_smo1.pmp_akt_mek_V1.init_get_report_smp(:periodStr, :lpuId, :parcelId, null, null))").setParameter("lpuId", lpuId).setParameter("period", period).setParameter("parcelId", parcelId).list());
		}
		List<Object[]> table1ObjectList = Db.select(sessionFactory, session -> session.createSQLQuery("select r_n,period,lpu_id,parcel_id,prof,usl_ok,opl_qt,opl_sum,err_qt,err_sum,itog_qt,itog_sum,last_day,ds from table (pmp_smo1.pmp_akt_mek_v1.get_tbl1(:periodStr, :lpuId, :parcelId))").setParameter("lpuId", lpuId).setParameter("period", period).setParameter("parcelId", parcelId).list());
