...
public class RegisterOfConclusionsBasedOnTheResultsOfMedicalAndEconomicControlWord {
	...
	public void create(Long lpuId, Date period, Long parcelId, String path) throws IOException {
		...
		List<Object[]> secondTableList = Db.select(smoSessionFactory, session -> session.createSQLQuery("select usl_ok,fil_id,bill_id,'iotd' as iotd,date_mm,sn_pol,cod_terr,err_code_1,service_sum,cod_fine,err_other from table(pmp_reestr_akt_v1.GET_tbl2(:periodStr, :lpuId, :parcelId, null, null))").setParameter("lpuId", lpuId).setParameter("periodStr", periodStr).setParameter("parcelId", parcelId).list());
		List<Object[]> thirdTableList = Db.select(smoSessionFactory, session -> session.createSQLQuery("select fil_id,'iotd' as iotd,bill_id,quarter,sum_pre,sum_fil,sum_pre as sum_pre2,sum_all,0 as sum_ from table(pmp_reestr_akt_v1.GET_tbl3(:periodStr, :lpuId, :parcelId, null, null))").setParameter("lpuId", lpuId).setParameter("periodStr", periodStr).setParameter("parcelId", parcelId).list());
