...
public class RegisterOfConclusionsBasedOnTheResultsOfMedicalAndEconomicControlWord {
	...
	public void create(Long lpuId, Date period, Long parcelId, String path) throws IOException {
		...
		List<Object[]> thirdTableList = Db.select(smoSessionFactory, session -> session.createSQLQuery("select fil_id,iotd,bill_id,quarter,sum_pre,sum_fil,sum_pre as sum_pre2,sum_all,0 as sum_,qt_all,sum_all,qt_all_1,sum_all_1,qt_all_2,sum_all_2,qt_all_3,sum_all_3,qt_all_4,sum_all_4 from table(pmp_reestr_akt_v1.get_tbl3(:periodStr, :lpuId, :parcelId, null, null))").setParameter("lpuId", lpuId).setParameter("periodStr", periodStr).setParameter("parcelId", parcelId).list());
