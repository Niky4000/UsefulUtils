 ...
 *
 *  Contributors:
 *     Ben Konrath <ben@bagu.org> - initial implementation Red Hat Incorporated -
 * improvements based on comments from JDT developers IBM Corporation - Code
 * review and integration IBM Corporation - Fix for 340181
 * *****************************************************************************
 */
 ...
 * configuration file to use to set the code formatter's options.</p>
 *
 * <p>
...
public class CodeFormatterApplication2 implements IApplication {
...
//            TextEdit edit = codeFormatter.format(CodeFormatter.K_COMPILATION_UNIT, contents, 0, contents.length(), 0, null);
    ...
    private Properties readConfig(String filename) {
        ...
        try {
            String configs = new String(Files.readAllBytes(configFile.toPath()));
            List<String> configsList = Arrays.asList(configs.split("\n")).stream().filter(str -> str.startsWith("<setting")).map(line -> line.replace("<setting id=\"", "").replace("\" value=\"", "=").replace("\"/>", "").replace("\r", "")).collect(Collectors.toList());
            ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(String.join("\n", configsList).getBytes());
            stream = new BufferedInputStream(byteArrayInputStream);
